...
chrome.runtime.onInstalled.addListener(function (details) {
    // Code à exécuter lors de l'installation ou de la mise à jour de l'extension
    if (details.reason === "install") {
      // Code spécifique à l'installation de l'extension
      console.log("L'extension a été installée.");
    } else if (details.reason === "update") {
      // Code spécifique à la mise à jour de l'extension
      console.log("L'extension a été mise à jour.");
    }
  });
  
  // Écoute l'événement de navigation complète
  chrome.webNavigation.onCompleted.addListener(function (details) {
    // Vérifier si l'URL correspond à YouTube
    if (
      /https:\/\/(www|m)\.youtube\.com/.test(details.url) ||
      /https:\/\/m\.youtube\.com/.test(details.url)
    ) {
      // Exécuter le script content.js sur la page
  ...
  // Fonction pour exécuter le script content.js sur la page
  function runContentScript() {
    // Exécutez ici le contenu du script que vous souhaitez exécuter sur les pages YouTube
    let ogVolume = 1;
    let pbRate = 1;
    ...
    setInterval(() => {
      if (document.getElementsByClassName("video-stream html5-main-video")[0] !== undefined) {
        ...
        if (
          document.getElementsByClassName("style-scope ytd-item-section-renderer sparkles-light-cta")[0] !== undefined
        ) {
